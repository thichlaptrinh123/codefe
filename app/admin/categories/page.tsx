...
import { Category, CategoryType } from "../components/category/category-types"
...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    // ✅ Nếu đã chọn loại bằng tay hoặc đang ở chế độ sửa -> không auto detect nữa
    if (hasManuallySelectedType || editingCategory) return;
  
    if (!formData.name) return;
  
    const nameLower = formData.name.toLowerCase();
  
    // --- logic dò loại danh mục ---
    const matchedType = categoryTypes.reduce((bestMatch, type) => {
      const typeName = type.name.toLowerCase();
      if (nameLower.includes(typeName)) {
        if (!bestMatch || typeName.length > bestMatch.name.length) {
          return type;
        }
      }
      return bestMatch;
  ...
  }, [formData.name, categoryTypes, hasManuallySelectedType, editingCategory]);
  ...
  const handleCancelEdit = () => {
    ...
    setHasManuallySelectedType(false); // ✅ cho phép auto detect lại
  };
  ...
  const handleAddOrUpdate = async () => {
    ...
    if (!matchedType && !hasManuallySelectedType) {
    ...
    const payload = {
      ...
      typeId: matchedType && !hasManuallySelectedType
        ? matchedType._id
        : formData.typeId, // nếu đã chọn tay hoặc không tìm thấy match thì lấy từ formData
    };
  ...
  const getFilteredCategories = () => {
    return categories.filter((cat) => {
      ...
      const catTypeId =
  cat.typeId !== null && cat.typeId !== undefined && typeof cat.typeId === "object"
    ? cat.typeId._id
    : cat.typeId;
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchTerm, filterStatus, filterTypeId]);
            ...
            <div>
              ...
              <select
                  ...
                  onChange={(e) => {
                    ...
                    setHasManuallySelectedType(true); // ✅ đánh dấu là đã chọn thủ công
                  }}
