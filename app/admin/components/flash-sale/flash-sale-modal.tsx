...
useEffect(() => {
  ...
  function isCategoryObject(
    category: unknown
  ): category is { _id: string } {
    return (
      typeof category === "object" &&
      category !== null &&
      "_id" in category &&
      typeof (category as any)._id === "string"
    );
  }
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      const matchSearch = product.name
        ...
        const productCategoryId = isCategoryObject(product.category)
...
const handleChange = (
  ...
  const handleSubmit = () => {
    ...
    if (form.discount_percent <= 0 || form.discount_percent > 50) {
    ...
    if (!form.id_product.length) {
      toast.warning("Vui lòng chọn ít nhất một sản phẩm!");
      return;
    }

    onSave({ ...(flashSale ?? {}), ...form }); // preserve ID if editing
    onClose();
  };
      ...
      {isEdit && (
        <div>
          ...
          {(() => {
            ...
            const startDate = form.start_datetime
              ? new Date(form.start_datetime)
              : null;
            const endDate = form.end_datetime
              ? new Date(form.end_datetime)
              : null;

            const isWithinRange =
              startDate && endDate && nowVN >= startDate && nowVN <= endDate;
