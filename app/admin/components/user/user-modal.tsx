...
export default function UserModal({
  ...
  const isDuplicatePhone = users.some(
    (u) =>
      u.phone === form.phone &&
      u.id !== initialData?.id // bỏ qua nếu đang chỉnh chính mình
  );
  ...
  if (isDuplicatePhone) {
    Swal.fire({
      ...
      text: "Số điện thoại này đã được sử dụng bởi người dùng khác.",
      confirmButtonColor: "#d33",
    });
  ...
  if (isDuplicateEmail) {
    Swal.fire({
      icon: "error",
      title: "Trùng email",
      text: "Email này đã được sử dụng bởi người dùng khác.",
      confirmButtonColor: "#d33",
    });
    return;
  }
