...
const [selectedProduct, setSelectedProduct] = useState<RawProduct | null>(null);
  ...
  const filteredProducts = useMemo(() => {
    ...
    const rawDigits = searchLower.replace(/[^\dk]/g, "");
  
    const statusOrder: Record<ProductStatus, number> = {
      active: 1,       // Hiển thị
      "sold-out": 2,   // Hết hàng
      "low-stock": 3,  // Sắp hết hàng
      inactive: 4,     // Tạm ngưng
    };
    ...
    return products
      .filter((product) => {
        ...
        const matchCategory = (product.categoryName ?? "").toLowerCase().includes(searchLower);
        const matchStock = String(product.stock ?? "").includes(searchLower);
  
        const price = Number(product.price ?? 0);
        const priceStr = price.toString();
        const priceFormatted = price.toLocaleString("vi-VN");
        const priceWithVnd = `${priceFormatted} vnđ`;
